...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
  ...
  let nftProvider: Capability<&MetaPanda.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
  ...
  prepare(dapp: AuthAccount, seller: AuthAccount) {
    ...
    let nftProviderPrivatePath = /private/MetaPandaCollectionProviderForNFTStorefront
    let hasLinkedCollection = seller
      .getCapability<&MetaPanda.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(
        nftProviderPrivatePath
      )!.check()
    if !hasLinkedCollection {
      seller.link<&MetaPanda.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(
        nftProviderPrivatePath,
    ...
    if existingOffers.length > 0 {
      for listingResourceID in existingOffers {
        ...
        if listing != nil && listing!.getDetails().nftID == saleItemID && listing!.getDetails().nftType == Type<@MetaPanda.NFT>(){
  ...
  pre {
    self.dappAddress == 0xf2af175e411dfff8: "Requires valid authorizing signature"
  }
  ...
  execute {
    ...
    let royaltyRecipient = getAccount(0xf2af175e411dfff8)
